...
2026-09-22 기준 배포 준비 기록. 저장소는 `Xeschen/poe2-farming`, 게시 예정 주소는 https://xeschen.github.io/poe2-farming/ 이다. 이 작업에서 원격 업로드나 Pages 활성화·공개 배포는 실행하지 않았다.

## 공개할 구성

공개 사이트는 HTML·CSS·JavaScript와 데이터만 제공한다. `npm run build`는 이전 `dist/`를 비운 뒤 `scripts/release-files.mjs`의 허용 목록 21개 파일만 생성한다. 현재 크기는 약 0.98 MiB다.
...
- 일부 대가 구성은 기록된 능력이 4개 미만이다. 자료에서 확인한 능력만 제공하며, 빈자리를 추측으로 채우지 않는다. 모든 빈자리가 필수 능력 누락을 뜻하지는 않는다.
