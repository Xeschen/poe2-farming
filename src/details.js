import { investmentText } from './model.js';
import { reviewLabel } from './catalog.js';
import { tabletCard, waystoneCard, atlasIcons } from './visual-setup.js';
...
export function renderDetail(m, sources, personal, preview, ctx) {
  const { esc, rich, term, list, evidence, ext, record, origin, changed, selected, compareIds, breadcrumb, pageLink } = ctx;
  ...
  const atlasSetup = `<section class="setup-group"><h4>아틀라스 선택</h4><p class="icon-help">아이콘에 마우스를 올리거나 눌러 선택 효과를 확인하세요.</p><ul class="atlas-nodes atlas-icon-strip">${atlasIcons(m.atlas.nodes, ctx)}</ul>${!m.atlas.nodes.length ? '<p class="muted">선택 미확인</p>' : ''}${m.atlas.otherPriorities?.length ? `<details class="setup-notes"><summary>추가 우선순위</summary>${list(m.atlas.otherPriorities)}</details>` : ''}${refs('아틀라스 근거', m.atlas.evidence)}<small class="setup-verification">전체 연결 경로: ${m.atlas.fullTreeVerified ? '확인 표시 있음 · 재검증 필요' : '미검증'}</small></section>`;
  const masterSetup = `<section class="setup-group master-group"><h4>대가 선택</h4>${m.masters.choices.length > 1 ? '<p class="setup-help">대안 중 하나의 구성을 사용하세요.</p>' : ''}<div class="master-options compact-master-options">${m.masters.choices.map((c, i) => `<article class="master-choice"><header class="master-choice-heading">${m.masters.choices.length > 1 ? `<span class="master-choice-label">선택안 ${i + 1}</span>` : ''}<h5>${term(c.name)}</h5></header><div class="master-ability-group" role="group" aria-label="${esc(c.name)}의 선택 능력">${renderMasterAbilities(c, { esc, term })}</div>${c.role ? `<details class="master-role-details"><summary>사용 조건</summary><p>${rich(c.role)}</p></details>` : ''}</article>`).join('') || '<p class="muted">구성 미확인</p>'}</div>${refs('대가 근거', m.masters.evidence)}<small class="setup-verification">전체 구성: ${m.masters.fullSetupVerified ? '영상 근거 있음 · 현재 유효성 미검증' : '미검증'}</small></section>`;
  const setup = panel('setup', '아틀라스·대가 설정', atlasSetup + masterSetup);
  const tablets = panel('tablets', '서판 구성', `${m.tablets.items.map((t, i) => tabletCard(t, i, preview, ctx)).join('') || `<p class="muted">${m.tablets.usage === 'not_used' ? '이 세팅에서는 서판을 사용하지 않습니다.' : '서판 구성 미확인'}</p>`}${m.tablets.notes.length ? `<details class="setup-notes"><summary>서판 사용 조건·참고</summary>${list(m.tablets.notes)}</details>` : ''}${refs('서판 근거', m.tablets.evidence)}`);
  const waystone = panel('waystone', '경로석·지도', `${waystoneCard(w, preview, ctx)}${w.notes.length ? `<details class="setup-notes"><summary>경로석 사용 조건·참고</summary>${list(w.notes)}</details>` : ''}<div class="map-summary"><h4>지도·환경</h4><div class="map-tags">${[...map.regions, ...map.biomes].map(name => `<span>${term(name)}</span>`).join('') || '<span class="muted">지도 조건 미확인</span>'}</div></div>${map.conditions.length || map.optional.length ? `<details class="setup-notes"><summary>지도 조건·선택 사항</summary>${map.conditions.length ? list(map.conditions) : ''}${map.optional.length ? `<h4>선택 사항</h4>${list(map.optional)}` : ''}</details>` : ''}${refs('경로석·지도 근거', [...w.evidence, ...map.evidence])}`);
  ...
  const layoutControls = preview ? '' : `<div class="detail-view-controls" role="group" aria-label="열람 레이아웃"><button type="button" data-detail-layout-choice="comfortable" aria-pressed="true" aria-label="기본형" title="기본형"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="16" height="6" rx="1"/><rect x="4" y="14" width="16" height="6" rx="1"/></svg></button><button type="button" data-detail-layout-choice="compact" aria-pressed="false" aria-label="컴팩트형" title="컴팩트형"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="7" height="6" rx="1"/><rect x="14" y="4" width="7" height="6" rx="1"/><rect x="3" y="14" width="7" height="6" rx="1"/><rect x="14" y="14" width="7" height="6" rx="1"/></svg></button></div>`;
  ...
  return `${breadcrumb([pageLink('파밍 라이브러리', 'library'), ...(personal ? [pageLink('내 데이터', 'library', { scope: 'personal' })] : []), `<span aria-current="page">${esc(m.name)}</span>`])}
    ...
    <div class="detail-layout"><div class="detail-main">${tablets}${waystone}${setup}${steps}${sourcePanel}${personal ? panel('notes','나의 메모',`<p style="white-space:pre-wrap">${esc(m.notes || '아직 작성한 메모가 없습니다.')}</p>`) : ''}</div>${preview ? '' : `<nav class="detail-toc" aria-label="파밍법 목차">${layoutControls}${toc.map(([key,label]) => `<a href="#${key}" data-detail-section="${key}">${label}</a>`).join('')}<button data-action="play">준비·진행</button></nav>`}</div>`;
}
